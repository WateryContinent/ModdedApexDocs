...
Apex Legends uses a PBR (Physically Based Rendering) shading pipeline. The Source engine ships with Blinn-Phong Shading (also known as Phong Shading, from Vietnamese computer graphics pioneer Bui Tuong Phong). Respawn modified Source to support PBR shading.
...
- slot1: _nml (normal map - [Note: these mimic light interaction with real surface geometry, to simulate depth, surface details, etc.](https://docs.unity3d.com/6000.3/Documentation/Manual/StandardShaderMaterialParameterNormalMap.html)) - MUST BE .DDS BC5U (DirectDraw Surface BC5 Unsigned, yellowish tint instead of purple-blue)
